...
function validateAuthorField(pluginManifest, pluginName) {
  const author = pluginManifest.author;
  if (typeof author === "string") {
    if (author.length === 0) {
      addError(`${pluginName}: "author" must not be empty.`);
    }
    return;
  }

  if (!author || typeof author !== "object" || typeof author.name !== "string" || author.name.length === 0) {
    addError(`${pluginName}: "author" must be a non-empty string or an object with a "name" field.`);
  }
}

function validateRequiredManifestFields(pluginManifest, pluginName) {
  for (const field of requiredPluginFields) {
    if (field === "author") {
      validateAuthorField(pluginManifest, pluginName);
      continue;
    }

    const value = pluginManifest[field];
    if (value === undefined || value === null || value === "") {
      addError(`${pluginName}: required manifest field "${field}" is missing or empty.`);
      continue;
    }

    if (field === "keywords" && (!Array.isArray(value) || value.length === 0)) {
      addError(`${pluginName}: "keywords" must be a non-empty array.`);
    }
  }
}

async function validateMcpConfig(pluginDir, pluginManifest, pluginName) {
  const mcpServerPaths = extractPathValues(pluginManifest.mcpServers);
  if (mcpServerPaths.length === 0) {
    return;
  }

  for (const mcpPathValue of mcpServerPaths) {
    const resolved = path.resolve(pluginDir, mcpPathValue);
    const exists = await pathExists(resolved);
    if (!exists) {
      addError(`${pluginName}: mcpServers references missing path "${mcpPathValue}".`);
      continue;
    }

    const mcpConfig = await readJsonFile(resolved, `${pluginName} MCP config`);
    if (!mcpConfig?.mcpServers || Object.keys(mcpConfig.mcpServers).length === 0) {
      addError(`${pluginName}: MCP config at "${mcpPathValue}" must define at least one mcpServers entry.`);
    }
  }
}

async function validatePlugin(pluginDir, pluginName, { enforceMarketplaceName = null } = {}) {
  ...
  if (enforceMarketplaceName && pluginManifest.name && pluginManifest.name !== enforceMarketplaceName) {
    addError(
      `${pluginName}: marketplace entry name does not match plugin.json name ("${pluginManifest.name}").`
    );
  }

  validateRequiredManifestFields(pluginManifest, pluginName);
  ...
  await validateMcpConfig(pluginDir, pluginManifest, pluginName);
  ...
  const declaredMcpPaths = extractPathValues(pluginManifest.mcpServers);
  const hasLegacyMcp = await pathExists(legacyMcpPath);
  let hasDeclaredMcp = false;
  for (const mcpPathValue of declaredMcpPaths) {
    if (await pathExists(path.resolve(pluginDir, mcpPathValue))) {
      hasDeclaredMcp = true;
      break;
    }
  }

  if (!hasLegacyMcp && declaredMcpPaths.length === 0) {
    addWarning(`${pluginName}: no mcp.json or mcpServers manifest field found (only needed when using MCP servers).`);
  } else if (declaredMcpPaths.length > 0 && !hasDeclaredMcp) {
    addWarning(`${pluginName}: mcpServers is declared but no referenced MCP config file was found.`);
  }
}
...
async function validateMarketplaceLayout() {
  ...
  for (const [index, entry] of marketplace.plugins.entries()) {
    ...
    await validatePlugin(pluginDir, entry.name, { enforceMarketplaceName: entry.name });
  }
...
async function validateSinglePluginLayout() {
  ...
  if (!pluginManifest) {
    addError(
      "No plugin manifest found. Expected .cursor-plugin/plugin.json at repo root, or .cursor-plugin/marketplace.json for multi-plugin repos."
    );
    return;
  }
